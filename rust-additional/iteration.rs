...
fn container_iterators() {
  ...
  */
  ...
  let v3 = std::vec::Vec::from_slice(&[6,7,8]);
  for i in v3.iter() {
    print!("{:d}", *i);
  }
  println!("rev_iter:");
  ...
  println!("move_iter:");
  let v5 = std::vec::Vec::from_slice(&[~"a", ~"b"]);
...
fn iterator_adaptors() {
...

  {
   ...
   (||{
     ...
     let ys = xs.rev_iter().skip(1).map(|&x| x * 2).collect::<std::vec::Vec<int>>();
     assert_eq!(ys, std::vec::Vec::from_slice(&[10,6,4,2,2,0]));
   })();
  ...
  (||{
    ...
    let xs = [1,2,3,4,5];
    let ys = std::vec::Vec::from_slice(&[7,9,11]);
